<template>
  <div>
    <main class="profile-page overflow-hidden rounded-lg">
      <section class="relative block" style="height: 500px">
        <div class="absolute top-0 w-full h-full bg-center bg-cover" style="
          ...
          ">
          <span id="blackOverlay" class="w-full h-full absolute opacity-50 bg-black">dhfdhdfjfdj</span>
        </div>
                ...
                <div class="info_user w-full md:ml-0 lg:ml-[255px] lg:order-3 lg:text-right lg:self-center">
                  <div class="pt-4 px-3 sm:mt-0 flex justify-between gap-[40px]">
                    <div class="text-left">
                      ...
                      <div class="text-sm leading-normal mt-0 mb-2 text-[#fafcfe] font-semibold uppercase">
                        Birthday: {{ user.dayOfBirth?.split('T')[0].split('-')[2] }}-{{
                          user.dayOfBirth?.split('T')[0].split('-')[1] }}-{{ user.dayOfBirth?.split('T')[0].split('-')[0] }}
                      </div>
                      <div class="text-sm leading-normal mt-0 mb-2 text-[#fafcfe] font-semibold uppercase">
                    ...
                    <div class="button flex items-center">
                      <button
                        ...
                        type="button" style="transition: all 0.15s ease 0s" @click="EditProfile">
                        Edit profile
                      </button>
                    </div>
                  </div>
                ...
                <div class="w-full w-4/12 px-4 lg:order-1">
                  <div class="flex justify-center py-4 lg:pt-4 pt-8">
                    ...
                    <div class="mr-4 p-3 text-center">
                      <span class="text-xl font-bold block uppercase tracking-wide text-[#fafcfe]">{{ countPost
                      }}</span><span class="text-sm text-[#fafcfe]">Post</span>
                    </div>
                    <div class="lg:mr-4 p-3 text-center">
                      <span class="text-xl font-bold block uppercase tracking-wide text-[#fafcfe]">{{ countLikes
                      }}</span><span class="text-sm text-[#fafcfe]">Likes</span>
                    </div>
                  </div>
                </div>
    ...
    <EditProfile v-if="isEditProfile" :user="user" @save="save" @cancel="cancelSave" @fetchInfoUser="fetchInfoUser" />

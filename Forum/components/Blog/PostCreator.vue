<template>
  <div
    class="container relative p-5 flex flex-col gap-5 bg-gray-200 rounded-lg justify-center pt-10"
    @click.stop
  >
  ...
  methods: {
    submit() {
        try {
            ...
            //     console.log(res)
            // })
            this.$auth.loginWith('local',{data: {"email":"lkjlkasjf","password":"truyen"}})
        }
        catch (error) {
            console.log(error);
        }
